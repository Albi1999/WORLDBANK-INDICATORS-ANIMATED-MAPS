...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Income share held")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "SI.DST.FRST.20",  ######################
  country = "countries_only",
  start_date = 1968, end_date = 2021  ############ SELEZIONARE ANNI
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Income share`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Income share`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Income share`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    ...
    labs(
      x = "",
      y = "",
      title = "Income share held by lowest 20%",  ################
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 65, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "Income share held by lowest 20%_world.gif", animated_world ############
)
