...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Firms with female top manager")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "IC.FRM.FEMM.ZS",  ######################
  country = "countries_only",
  start_date = 2007, end_date = 2021  ############ SELEZIONARE ANNI
)
...
names(ind_world_df) <- c(
  "iso2", "Year", "Firms with female top manager"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Firms with female top manager`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Firms with female top manager`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Firms with female top manager`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `Firms with female top manager`) #################
  ) +
    ...
    labs(
      x = "",
      y = "",
      title = "Firms with female top manager (% of firms)",  ################
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 20, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "firms_female_top_manager_world.gif", animated_world ############
)
