...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Net migration")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "SM.POP.NETM",  ######################
  country = "countries_only",
  start_date = 1960, end_date = 2021  ############ SELEZIONARE ANNI
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Net migration`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Net migration`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Net migration`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
gganimate::anim_save(
  "Net migration_world.gif", animated_world ############
)
