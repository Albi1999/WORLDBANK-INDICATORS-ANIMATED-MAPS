...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Container port traffic")  ######################
fix(kpi_df)
...
names(ind_world_df) <- c(
  "iso2", "Year", "Container port traffic"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Container port traffic`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Container port traffic`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Container port traffic`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `Container port traffic`) #################
  ) +
...
gganimate::anim_save(
  "Container port traffic_world.gif", animated_world ############
)
