...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Investment in transport")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "IE.PPI.TRAN.CD",  ######################
  country = "countries_only",
  start_date = 1984, end_date = 2021  ############ SELEZIONARE ANNI
)
...
names(ind_world_df) <- c(
  "iso2", "Year", "Investment in transport"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Investment in transport`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Investment in transport`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Investment in transport`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `Investment in transport`) #################
  ) +
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 60, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "Investment in transport with private participation_world.gif", animated_world ############
)
