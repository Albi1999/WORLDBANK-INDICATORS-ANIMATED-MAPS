...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "technology exports")  ######################
fix(kpi_df)
...
names(ind_world_df) <- c(
  "iso2", "Year", "High technology exports"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`High technology exports`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`High technology exports`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`High technology exports`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `High technology exports`) #################
  ) +
...
gganimate::anim_save(
  "High_technology_exports_world.gif", animated_world ############
)
