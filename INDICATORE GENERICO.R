...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "forest area")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "AG.LND.FRST.ZS",  ######################
  country = "countries_only",
  start_date = 1990, end_date = 2020  ############ SELEZIONARE ANNI
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Forest Area`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Forest Area`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Forest Area`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
gganimate::anim_save(
  "forest_area_world.gif", animated_world ############
)
