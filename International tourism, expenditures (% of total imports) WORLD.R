...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "International tourism, expenditures")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "ST.INT.XPND.MP.ZS",  ######################
  country = "countries_only",
  start_date = 1995, end_date = 2020  ############ SELEZIONARE ANNI
)
...
names(ind_world_df) <- c(
  "iso2", "Year", "International tourism, expenditures"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`International tourism, expenditures`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`International tourism, expenditures`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`International tourism, expenditures`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `International tourism, expenditures`) #################
  ) +
    ...
    labs(
      x = "",
      y = "",
      title = "International tourism, expenditures\n(% of total imports)",  ################
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 40, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "International tourism, expenditures (% of total imports)_world.gif", animated_world ############
)
