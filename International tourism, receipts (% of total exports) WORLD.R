...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "International tourism, receipts")  ######################
fix(kpi_df)
...
names(ind_world_df) <- c(
  "iso2", "Year", "International tourism, receipts"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`International tourism, receipts`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`International tourism, receipts`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`International tourism, receipts`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `International tourism, receipts`) #################
  ) +
...
gganimate::anim_save(
  "International tourism, receipts (% of total exports)_world.gif", animated_world ############
)
