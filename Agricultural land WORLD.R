...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Agricultural land")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "AG.LND.AGRI.ZS",  ######################
  country = "countries_only",
  start_date = 1961, end_date = 2020  ############ SELEZIONARE ANNI
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Agricultural land`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Agricultural land`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Agricultural land`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 70, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "Agricultural land_world.gif", animated_world ############
)
