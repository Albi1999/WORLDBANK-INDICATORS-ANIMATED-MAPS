...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Population living in slums")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "EN.POP.SLUM.UR.ZS",  ######################
  country = "countries_only",
  start_date = 2000, end_date = 2020  ############ SELEZIONARE ANNI
)
...
names(ind_world_df) <- c(
  "iso2", "Year", "Population living in slums"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Population living in slums`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Population living in slums`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Population living in slums`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `Population living in slums`) #################
  ) +
...
animated_world <- gganimate::animate(
  timelapse_world_map,
  nframes = 120,
  duration = 25, ###### VARIA IN BASE AL NUMERO DI ANNI
  start_pause = 3,
  end_pause = 30,
  height = 6,
  width = 7.15,
...
gganimate::anim_save(
  "Population living in slums (% of urban population)_world.gif", animated_world ############
)
