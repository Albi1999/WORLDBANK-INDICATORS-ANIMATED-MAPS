...
# CARICAMENTO DATI ----
## WB DATA ----
kpi_df <- wbstats::wb_search(pattern = "Poverty headcount ratio")  ######################
fix(kpi_df)

ind_df <- wbstats::wb_data(
  indicator = "SI.POV.NAHC",  ######################
  country = "countries_only",
  start_date = 1985, end_date = 2021  ############ SELEZIONARE ANNI
)
...
names(ind_world_df) <- c(
  "iso2", "Year", "Poverty headcount ratio"   ######################
)
...
# BREAKS ----
vmin <- min(ind_world_sf$`Poverty headcount ratio`, na.rm = TRUE, finite = TRUE) ################
vmax <- max(ind_world_sf$`Poverty headcount ratio`, na.rm = TRUE, finite = TRUE) ################
brk <- round(classInt::classIntervals(
  ind_world_sf$`Poverty headcount ratio`, ###############
  n = 6,
  style = "fisher"
)$brks, 1) |>
  head(-1) |>
...
get_animated_world_map <- function() {
  world_map <- ggplot(
    data = ind_world_sf,
    aes(fill = `Poverty headcount ratio`) #################
  ) +
...
gganimate::anim_save(
  "Poverty headcount ratio at national poverty lines_world.gif", animated_world ############
)
